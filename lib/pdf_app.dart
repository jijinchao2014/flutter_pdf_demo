...
class MyPDFPage extends StatefulWidget {
  ...
  @override
  State<MyPDFPage> createState() => _PDFPageState();
}

class _PDFPageState extends State<MyPDFPage> {
  ...
  Future<void> loadImage() async {
    netImage = await networkImage('https://www.nfet.net/nfet.jpg');
  ...
  void generateDocument(PdfPageFormat format) async {
    ...
    doc.addPage(pw.MultiPage(
        ...
        header: (pw.Context context) {
          ...
          return pw.Container(
              ...
              decoration: const pw.BoxDecoration(
                  border: pw.Border(
                      bottom:
                          pw.BorderSide(width: 0.5, color: PdfColors.green))),
        ...
        build: (pw.Context context){
          ...
          // return FutureBuilder(builder: (BuildContext context, AsyncSnapshot<List<pw.Widget>> snapshot){
          //   return buildContent(context);
          // });
        }));

    List<int> bytesd = await doc.save();
    js.context['pdfData'] = base64.encode(bytesd);
    js.context['filename'] = 'JIJINCHAO.pdf';
    Timer.run(() {
...
List<pw.Widget> buildContent(pw.Context context) {
    List<pw.Widget> aa = [];
    for (int i = 0; i < 100; i++) {
      aa.add(pw.Image(netImage));
      aa.add(pw.Header(
          level: 0,
          title: '$i-----Portable Document Format111',
          child: pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: <pw.Widget>[
                pw.Text('$i-----Portable Document Format222',
                    textScaleFactor: 2),
                pw.PdfLogo()
              ])));
      aa.add(pw.Paragraph(
          text:
              '$i---The Portable Document Format (PDF) is a file format developed by Adobe in the 1990s to present documents, including text formatting and images, in a manner independent of application software, hardware, and operating systems. Based on the PostScript language, each PDF file encapsulates a complete description of a fixed-layout flat document, including the text, fonts, vector graphics, raster images and other information needed to display it. PDF was standardized as an open format, ISO 32000, in 2008, and no longer requires any royalties for its implementation.'));
      aa.add(pw.Paragraph(
          text:
              '$i---Today, PDF files may contain a variety of content besides flat text and graphics including logical structuring elements, interactive elements such as annotations and form-fields, layers, rich media (including video content) and three dimensional objects using U3D or PRC, and various other data formats. The PDF specification also provides for encryption and digital signatures, file attachments and metadata to enable workflows requiring these features.'));
    }
    ...
    //   pw.Header(
    //       level: 0,
    //       title: 'Portable Document Format111',
    //       child: pw.Row(
    //           mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
    //           children: <pw.Widget>[
    //             pw.Text('Portable Document Format222', textScaleFactor: 2),
    //             pw.PdfLogo()
    //           ])),
    //   pw.Paragraph(
    //       text:
  ...
  // Future<pw.ImageProvider> buildImage() async {
  //    final image = await ImageImage("https://image.baidu.com/search/detail?ct=503316480&z=0&ipn=d&word=%E5%9B%BE%E7%89%87&hs=0&pn=3&spn=0&di=7214885350303334401&pi=0&rn=1&tn=baiduimagedetail&is=0%2C0&ie=utf-8&oe=utf-8&cl=2&lm=-1&cs=1819503313%2C1403041210&os=4134277524%2C3201373319&simid=3503472406%2C402338039&adpicid=0&lpn=0&ln=30&fr=ala&fm=&sme=&cg=&bdtype=0&oriquery=%E5%9B%BE%E7%89%87&objurl=http%3A%2F%2Fimg.daimg.com%2Fuploads%2Fallimg%2F200630%2F1-200630154924.jpg&fromurl=ippr_z2C%24qAzdH3FAzdH3Fooo_z%26e3B1wt42_z%26e3Bv54AzdH3Fri5p5AzdH3FdadaamAzdH3Fri5p5_80bcma_z%26e3Bip4s&gsm=&islist=&querylist=&dyTabStr=MCwxLDYsMyw0LDUsMiw3LDgsOQ%3D%3D");
  //   return image;
  // }
}
